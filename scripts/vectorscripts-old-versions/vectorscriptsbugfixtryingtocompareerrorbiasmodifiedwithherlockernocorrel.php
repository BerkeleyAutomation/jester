		...
		{	
			...
			for ($jokeindex = 0; $jokeindex < $numjokes; $jokeindex++)
			{
				...
				if (isRated($connection, $userid, $jokeid))
				{
					...
					/* Added for Bias Error */
					if ($userid == 1 && $jokeid == 1)				
						print "meanclusterdevvector[$jokeindex] += " . ($jokerating - $usermeanrating) . " = ($jokerating - $usermeanrating)\n";
...
function calculateErrorBiasModified($connection)
{
	...
	clearTable($connection, "errorbiaspredictions");
	...
	while ($clusterindex < count($clusters))
	{
		...
		for ($i = 0; $i < count($useridsforclusters[$clusterindex]); $i++)
		{
			...
			for ($jokeindex = 0; $jokeindex < $numjokes; $jokeindex++) //Takes into account removed jokes
			{
				...
				$n = trimPrediction($usermeanrating + $clustermeandevs[$clusterindex][$jokeindex]);
				if ($userid == 1 && $jokeid == 1)				
					print "n = $usermeanrating + " . $clustermeandevs[$clusterindex][$jokeindex] . " = " . $n . "\n";
				$predictedrating = round($n, floor(0 - log10(abs($n))) + 6); //The predicted rating (for use in calculating the error)
				if ($userid == 1 && $jokeid == 1)	
					print "predictedrating = $predictedrating\n";
				
				$query = "INSERT INTO errorbiaspredictions (userid, jokeid, prediction) VALUES ({$userid}, {$jokeid}, {$predictedrating})";
				$result = mysql_query($query, $connection);
				if (!$result)
					die($_POST["preinvalidquery"] . mysql_error() . $_POST["postinvalidquery"]);
				...
				if (isRated($connection, $userid, $jokeid))
				{
					//print "User $userid, Joke $jokeid predicted rating: " . $predictedrating . "\n";
					$jokerating = getJokeRating($connection, $userid, $jokeid); //The actual rating
					
					//print "original predicted rating: $n\n";
					//print "predictedrating: $predictedrating\n";
					//print "maesumuser += abs($jokerating - $predictedrating)\n";
					$maesumuser += abs($jokerating - $predictedrating); //The sum used to calculate mean absolute error for this user
					//print "maesumuser = $maesumuser\n";
				}
			}
	...
	$maetotal = $maesumtotal / $usercount;
	print "maetotal = $maesumtotal / $usercount\n";
	$nmaetotal = $maetotal / ($maxjokerating - $minjokerating);
	
	print("\n");
	print("Modified Biased MAE (using updated cluster means): " . $maetotal . "\n");
	print("Modified Biased NMAE (using updated cluster means): " . $nmaetotal . "\n");
...
function dbCompare($connection)
{
	$predictionarray1 = array();
	$predictionarray2 = array();
	
	$query = "SELECT userid, jokeid, prediction FROM errorbiaspredictions ORDER BY userid, jokeid";
	$result = mysql_query($query, $connection);
	if (!$result)
   		die($_POST["preinvalidquery"] . mysql_error() . $_POST["postinvalidquery"]);
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		$predictionarray1[$userid][$jokeid] = $prediction;
	}
	
	$query = "SELECT userid, jokeid, prediction FROM herlockerpredictionsclusterednocorrel ORDER BY userid, jokeid";
	$result = mysql_query($query, $connection);
	if (!$result)
   		die($_POST["preinvalidquery"] . mysql_error() . $_POST["postinvalidquery"]);
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		$predictionarray2[$userid][$jokeid] = $prediction;
	}
	
	foreach ($predictionarray1 as $userid => $predictionsuser1)
	{
		foreach ($predictionsuser1 as $jokeid => $prediction)
		{
			if ($predictionarray1[$userid][$jokeid] != $predictionarray1[$userid][$jokeid])
			{
				print "error \n";
				print $predictionarray1[$userid][$jokeid] . " vs. " . $predictionarray2[$userid][$jokeid] . "\n";
			}
		}
	}
}
				...
				for ($j = 0; $j < count($useridsforclusters[$clusterindex]); $j++)
				{
					...
					*/
					...
					if (!($ratingvect[$useridj][$jokeid] === false))
					{
						...
						if ($useridi == 1 && $jokeid == 1)
						{
							print "num += " . (($jokerating - $userjmeanrating) * $userijcorrel) . "\n";
							print "denom += " . $userijcorrel;
						}
					}
				}
				
				$num2 = round($num, floor(0 - log10(abs($num))) + 6);
				if ($useridi == 1 && $jokeid == 1)
				{
					print "num2 = " . round($num, floor(0 - log10(abs($num))) + 6) . "\n";
				}

				if ($denom != 0)
					$prediction = trimPrediction($userimeanrating + ($num2 / $denom));
				else
					$prediction = trimPrediction($userimeanrating + ($num2 * $largeint));
...
function calculateErrorBiasHerlockerClusteredNoCorrel($connection)
{
		...
		if (inACluster($connection, $userid)) //Only consider users who are clustered, to match the sample of the other error algorithms
		{
			...
			for ($jokeindex = 0; $jokeindex < $numjokes; $jokeindex++)
			{
				...
				if (isRated($connection, $userid, $jokeid))
				{
					...
					//print "predictedrating($userid, $jokeid): $predictedrating\n";
					//print "maesumuser += abs($jokerating - $predictedrating)\n";
					$maesumuser += abs($jokerating - $predictedrating); //The sum used to calculate mean absolute error for this user
					//print "maesumuser = $maesumuser\n";
					$countrated++;
	...
	print("\n");
	print("Herlocker Biased and Clustered MAE with No Correlations (using updated cluster means): " . $maetotal . "\n");
	print("Herlocker Biased and Clustered NMAE with No Correlations  (using updated cluster means): " .$nmaetotal . "\n");
