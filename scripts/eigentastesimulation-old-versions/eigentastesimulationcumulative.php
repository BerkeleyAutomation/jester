...
runSimulationUsersRatedX($connection, $meanweight, $numjokestoratearray, $e2plot, $prede2plot, $pte2plot, $ptprede2plot, $e3plot, $prede3plot, $pte3plot, $ptprede3plot);
...
$plotarray[] = $prede2plot;
$plotnamearray[] = $prede2plotname;
$plotarray[] = $prede3plot;
...
$plotarray[] = $ptprede2plot;
$plotnamearray[] = $ptprede2plotname;
$plotarray[] = $ptprede3plot;
...
foreach ($numjokestoratearray as $numjokestorate)
{
	...
	$userquery = "SELECT e2simulationusers.userid FROM e2simulationusers, e3simulationusers WHERE e2simulationusers.numjokestorate = e3simulationusers.numjokestorate AND e2simulationusers.numjokestorate=50 AND e2simulationusers.userid = e3simulationusers.userid";
	runSimulationE2($connection, $numjokestorate, $userquery, $insert);
	runSimulationE3($connection, $meanweight, $numjokestorate, $userquery, $insert);
}
*/
...
function exportMATLABDataAllUsers($connection)
{
	...
	$query = "SELECT clusters.userid FROM clusters, users WHERE clusters.userid = users.userid AND users.numrated = 100 ORDER BY userid";
	$result = mysql_query($query, $connection);
	if (!$result)
		die($_POST["preinvalidquery"] . mysql_error() . $_POST["postinvalidquery"]);
	...
	for ($jokeindex = 0; $jokeindex < $numjokes; $jokeindex++)
	{
		...
		foreach ($usersrated100 as $userid)
		{
			$jokeratings[$jokeid][$userid] = isGetJokeRating($connection, $userid, $jokeid);
...
function runSimulationUsersRatedX($connection, $meanweight, &$numjokestoratearray, &$e2plot, &$prede2plot, &$pte2plot, &$ptprede2plot,
	&$e3plot, &$prede3plot, &$pte3plot, &$ptprede3plot)
{	
	//clearTable($connection, "e2simulationusers");
	//clearTable($connection, "e3simulationusers");
	...
	runSimulationE2UsersRatedX($connection, $numjokestoratearray, $e2plot, $prede2plot, $pte2plot, $ptprede2plot);
	//runSimulationE3UsersRatedX($connection, $meanweight, $numjokestoratearray, $e3plot, $prede3plot, $pte3plot, $ptprede3plot);
}

function runSimulationE2UsersRatedX($connection, &$numjokestoratearray, &$e2plot, &$prede2plot, &$pte2plot, &$ptprede2plot)
{	
	global $predictjokes;
	...
	{	
		...
		$predictionsforcluster = array_fill(1, 100, false);
		...
		*/
		...
		while ($jokeratingcount[$maxnumjokestorate] < $maxnumjokestorate)
		{
			foreach ($numjokestoratearray as $numjokestorate)
			{
				...
				{
					//Find the highest rated joke for this cluster			
					$recommendedjokeid = getHighestUnratedJokeIDInCluster($userid, $predictvectors[$cluster], $ratedjokes[$numjokestorate]);
	...
	foreach ($numjokestoratearray as $numjokestorate)
	{
		...
		*/
		...
		$e2plot .= "$meanmeanjokerating ";
		$prede2plot .= "$meanmeanpredictedrating ";
		
		$pte2plot .= "$ptmeanjokerating ";
		$ptprede2plot .= "$ptmeanpredictedrating ";
	
		print("\nAveraged Ratings of First $numjokestorate Jokes (Eigentaste 2.0): $meanmeanjokerating\n");
		print("\nAveraged Predicted Ratings of First $numjokestorate Jokes (Eigentaste 2.0): $meanmeanpredictedrating\n");
		print("User Count: $usercount\n");
	}
}

function runSimulationE3UsersRatedX($connection, $meanweight, &$numjokestoratearray, &$e3plot, &$prede3plot, &$pte3plot, &$ptprede3plot)
{		
	global $predictjokes;
	...
	foreach ($numjokestoratearray as $numjokestorate)
	{
		...
		$e3plot .= "$meanmeanjokerating ";
		$prede3plot .= "$meanmeanpredictedrating ";
		
		$pte3plot .= "$ptmeanjokerating ";
		$ptprede3plot .= "$ptmeanpredictedrating ";
...
{
	$cumulativeratingssum = array();
	$cumulativeratingscount = array();
	
	while ($cumulativeratingscount[$maxnumjokestorate] < $maxnumjokestorate)
	{
		foreach ($numjokestoratearray as $numjokestorate)
		{
			if ($cumulativeratingscount[$numjokestorate] < $numjokestorate)
			{
				$cumulativeratingssum[$numjokestorate] += $pointwiseratings[$numjokestorate];
...
function calculateEigentastePredictionsForClusters($connection)
{
	...
	while ($row = mysql_fetch_array($result))
	{
		...
		$predictions[$cluster] = array_fill(1, 100, false);
