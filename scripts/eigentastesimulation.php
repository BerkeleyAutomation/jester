...
define("NUMJOKES", 5);
define("NUMPREDICTJOKES", 1);
define("NUMNONPREDICTJOKES", 4);
/* End */
...
//exportMATLABDataAllUsersClusterMeans($connection);
...
function runSimulation($connection)
{
	...
	$meanweight = 5;
	$movingaveragesize = 5;
	$initialratedjokes = $predictjokes; //Copy
	...
	runSimulationUsersRatedX($connection, $meanweight, $movingaveragesize, $numjokestoratearray, $e2plot, $prede2plot, $pte2plot, $ptprede2plot, $e3plot, $prede3plot, $pte3plot, $ptprede3plot, $se2plot, $sprede2plot, $se3plot, $sprede3plot, $initialratedjokes);
	...
	$plotarray[] = getPlotString(diffArray($pte3plot, $pte2plot));
	$plotnamearray[] = "diff_pte3plot_pte2plot";
	
	$plotarray[] = getPlotString(diffArray($e3plot, $e2plot));
	$plotnamearray[] = "diff_e3plot_e2plot";
	
	$plotarray[] = getPlotString(diffArray($pte2plot, $ptprede2plot));
	$plotnamearray[] = "diff_pte2plot_ptprede2plot";
	
	$plotarray[] = getPlotString(diffArray($pte3plot, $ptprede3plot));
	$plotnamearray[] = "diff_pte3plot_ptprede3plot";
	
	$plotarray[] = getPlotString(diffArray($e2plot, $prede2plot));
	$plotnamearray[] = "diff_e2plot_prede2plot";
	
	$plotarray[] = getPlotString(diffArray($e3plot, $prede3plot));
	$plotnamearray[] = "diff_e3plot_prede3plot";
...
function exportMATLABDataAllUsersClusterMeans($connection)
{
	global $numjokes;
...
/* Tested - works */
function runSimulationUsersRatedX($connection, $meanweight, $movingaveragesize, &$numjokestoratearray, &$e2plot, &$prede2plot, &$pte2plot, &$ptprede2plot,
	&$e3plot, &$prede3plot, &$pte3plot, &$ptprede3plot, &$se2plot, &$sprede2plot, &$se3plot, &$sprede3plot, $initialratedjokes)
{	
	$maxnumjokestorate = max($numjokestoratearray);

	calculateEigentastePredictionsForClusters($connection);
	
	runSimulationE2UsersRatedX($connection, $numjokestoratearray, $pte2plot, $ptprede2plot, $initialratedjokes);
	runSimulationE3UsersRatedX($connection, $meanweight, $movingaveragesize, $numjokestoratearray, $pte3plot, $ptprede3plot, $initialratedjokes);
...
/* Tested - works */
function runSimulationE2UsersRatedX($connection, &$numjokestoratearray, &$pte2plot, &$ptprede2plot, $initialratedjokes)
{	
	...
	//Loop through all users who were not in the initial user group and have rated all the jokes
	...
	{	
		...
		$ratedjokes = $initialratedjokes;
		
		if (($usercount % 100) == 0)
			print "User count has passed $usercount...\n";
		
		//Match the user with a cluster
		$cluster = matchWithCluster_Copy($connection, $userid);
		...
		//Get the user's predicted ratings for all the jokes (using the matched cluster)
...
/* Tested - works */
function runSimulationE3UsersRatedX($connection, $meanweight, $movingaveragesize, &$numjokestoratearray, &$pte3plot, &$ptprede3plot, $initialratedjokes)
{		
	global $predictjokes, $numjokes;
	...
	{	
		...
		while ($row = mysql_fetch_array($resultinner))
		{
			...
			preloadJokeClusterRatings($connection, $cluster, $jokecluster, $jokeclustermean, $meanweight, $jokeclustersforjokes, $jokeclusterratings, $dbjokeratings, $ratedjokes);
		}
		...
		foreach ($numjokestoratearray as $numjokestorate)
		{
			...
			while ($recommendedjokeid === false)
			{
				...
				$favoritejokecluster = getFavoriteJokeCluster_Copy($jokeclusterratings, $movingaveragesize, $emptyclusters);
			...
			$jokeclusterratings[$favoritejokecluster][] = $jokerating;
...
/* Tested - works */
function preloadJokeClusterRatings($connection, $cluster, $jokecluster, $jokeclustermean, $meanweight, &$jokeclustersforjokes, &$jokeclusterratings, &$dbjokeratings, &$ratedjokes)
{
	//Added
	//(Consider the user's actual ratings of prediction jokes, rather than just the cluster means)
	$jokeclusterratings[$jokecluster] = array();
	...
	foreach ($ratedjokes as $ratedjokeid)
	{
		...
		{
			$jokeclusterratings[$jokecluster][] = $dbjokeratings[$ratedjokeid];
			$numratedjokesinjokecluster++;
		}
	}
	
	//If the array was (0 => 5.5, 1 => 2.5, 2 => 3.0), it is now (0 => 3.0, 1 => 2.5, 2 => 5.5)
	$jokeclusterratings[$jokecluster] = array_reverse($jokeclusterratings[$jokecluster]);

	$remainingratings = ($meanweight - $numratedjokesinjokecluster);
	
	if ($remainingratings < 0)
	{
		print "Error: the current meanweight value does not allow for all of the rated prediction jokes to be taken into account.\n";
		exit;
	}
	
	for ($remainingratingscount = 1; $remainingratingscount <= $remainingratings; $remainingratingscount++)
	{
		$jokeclusterratings[$jokecluster][] = $jokeclustermean;
	}
...
function diffArray(&$plotarray1, &$plotarray2)
{
	...
	foreach ($plotarray1 as $numjokestorate => $plotelement)
	{
		$plotarray1minus2[$numjokestorate] = ($plotarray1[$numjokestorate] - $plotarray2[$numjokestorate]);
	}
...
/* Tested - works */
function getFavoriteJokeCluster_Copy(&$jokeclusterratings, $movingaveragesize, &$emptyclusters)
{
	...
	{	
		...
		{
			print "Error: cannot have a moving average with zero elements!\n";
			exit;
		}
...
/*
//Used for unit testing
function matchWithCluster_Copy($connection, $userid)
{
	...
	{
		echo "Error: matchWithCluster_Copy should not have been called!\n";
		exit;
	}
}
*/
...
{	
	...
	$diffedarray = diffArray($array1, $array2);
	
	foreach ($diffedarray as $key => $arrayelement)
	{
		print "diffedarray[key] = $arrayelement\n";
	}
	
	/*
	//Test preloadJokeClusterRatings
	...
	print "jokeclusterratings[$jokecluster] = ";
	foreach ($jokeclusterratings[$jokecluster] as $jokeclusterrating)
	{
		print "$jokeclusterrating ";
	}
	print "\n";
	
	print "count(jokeclusterratings[$jokecluster]) = " . count($jokeclusterratings[$jokecluster]) . "\n";
	
	//Test new getFavoriteJokeCluster_Copy
	
	$jokeclusterratings = array();

	$jokeclusterratings[1][] = -9.9;
	$jokeclusterratings[1][] = -1.5;
	$jokeclusterratings[1][] = -6.7;
	$jokeclusterratings[1][] = 8.9;
	$jokeclusterratings[2][] = -9;
	$jokeclusterratings[3][] = 9;
	$jokeclusterratings[4][] = 9.9;
	$jokeclusterratings[4][] = 2.5;
	$jokeclusterratings[4][] = -6.7;
	$jokeclusterratings[4][] = -9.9;
	
	$emptyclusters = array(2, 3);
	
	$favoritejokecluster = getFavoriteJokeCluster_Copy($jokeclusterratings, 2, $emptyclusters);
	...
	//Test: getFavoriteJokeCluster_Copy
	...
	$favoritejokecluster = getFavoriteJokeCluster_Copy($jokeclustersums, $jokeclustercounts, $emptyclusters);
	...
	foreach ($useridarray as $userid)
	{
		...
		$matchedcluster = matchWithCluster_Copy($connection, $userid);
