...
$query = "SELECT userid FROM users WHERE numrated > 20 LIMIT 100";
...
{	
	...
	$query = "SELECT jokeid, jokerating FROM ratings WHERE userid={$userid} AND jokeid IN (5, 7, 8, 13, 15, 16, 17, 18, 19, 20)";
	...
	while ($row = mysql_fetch_array($resultinner))
	{
		...
		$ratingvect[$jokeindex] = $jokerating;
	}
	
	foreach ($ratingvect as $key => $rating) {
		print $rating . "\t";
	}
