...
<?php
/* To Do for this file:
	*- Add a prediction calculation function
	*- Go back through the possible bugs of eigentastesimulation.php, i.e. predictions of 0 vs. null, and fix those problems
	*- Make sure the simulation functions I copy/paste deal with removed jokes and joke seeding (because the simulation didn't need to)
	*- jokes.php has $status = 1 in two places, and in the second place, where people go from random to recommend, should preloading of jokeclusterratings occur? In random mode, are jokeclusterratings being set and such?
	*- all previously registered users must have usingjester4 = 1, usingjester5 = 1, and make sure that when they log back in they are presented stuff properly
	*- make sure that all instances of numjokes are replaced with numavailablejokes when necessary, and make sure everything ignores removed jokes when need be
	
	*- when launching, load jester4's most recent users/ratings/jokes data
	*- make sure jester4_clustermeans and jester5_clustermeans match at launch
	*- set all old users to usingjester4 = 1 and usingjester5 = 1 at launch
	
	- When updating joke clusters, a function needs to:
		a) Log all users off
		b) Recreate jokeclusters
		c) Delete all jokeclusterratings and recreate them by using initializeJokeClusterRatings (using new joke clusters)
			and then looping through all ratings since the point the recommendations started and adding those, as well, to jokeclusterratings with the updated jokecluster of each respective joke
		d) Delete all emptyjokeclusters (because they refer to the dated jokeclusters), and possibly repopulate
			the table by calculating which of the new joke clusters are empty (although this may or may not be necessary)
		e) Make users log back in to resume
*/

//This file was created as a duplicate of vectorscriptsbugfix.php at 4:03 PM on August 15, 2007
...
openConnection();
print("Running Jester 4.0 and 5.0 Offline Scripts\n\n");
...
print("\nRunning Jester 4.0 Offline Scripts\n");
...
print("\nRunning Jester 5.0 Offline Scripts\n");
clearArrays($connection);
...
print("\nJester 4.0 and 5.0 Offline Scripts Complete\n");
...
//For testing
function randomlyAssignOldUsers($connection)
{
	global $tablenames;
