...
bestGauge = function(SIZE) {
  ...
  print("OPTIMIZING GAUGE SET")
  for (i in 1:6) { # fix all the jokes from 1 to 6
    print(paste("Optimizing Joke #: ", i))
  ...
  c(optGauge, bestNMAE, bestNMAEOut)
}

optimize_dead = function() {
  best_nmae = 999
  best_nmae_out = 999
  best_size = 999
  best_gauge = c()
  for(i in seq(1, 4, 0.5)) {    
    print(paste("Dead Zone Size: ", i))
    result = bestGauge(i)
    opt_gauge = result[1:6]
    if(result[7] < best_nmae) {
      best_nmae = result[7]
      best_nmae_out = result[8] 
      best_size = i
      best_gauge = opt_gauge
    }
    print(c("Best size: ", best_size))
    print(c("Best gauge: ", best_gauge))
    print(c("NMAE: ", best_nmae))
    print(c("NMAE OUT:", best_nmae_out))
  }
}

enableJIT(3)

optimize_dead = cmpfun(optimize_dead)
bestGauge = cmpfun(bestGauge)
NMAE2_OUT = cmpfun(NMAE2_OUT)
NMAE_OUT = cmpfun(NMAE_OUT)
NMAE2 = cmpfun(NMAE2)
NMAE = cmpfun(NMAE)
generatePredictions = cmpfun(generatePredictions)
generateDifferences = cmpfun(generateDifferences)
calculateNMAE = cmpfun(calculateNMAE)
eigentaste2 = cmpfun(eigentaste2)
eigentaste = cmpfun(eigentaste)

optimize_dead_fine = function() {
  best_nmae = 999
  best_nmae_out = 999
  best_size = 999
  best_gauge = c()
  for(i in seq(0.75, 2, 0.25)) {    
    print(paste("Dead Zone Size: ", i))
    result = bestGauge(i)
    opt_gauge = result[1:6]
    if(result[7] < best_nmae) {
