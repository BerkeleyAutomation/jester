...
for (i in this) {
	...
	for (j in (i+1):length(gauge)) {
		...
		comp1 = as.vector(prin$loadings[,1] %*% t(traingauge))
		comp2 = as.vector(prin$loadings[,2] %*% t(traingauge))
		...
		comp1 = as.vector(prin$loadings[,1] %*% t(testgauge))
		comp2 = as.vector(prin$loadings[,2] %*% t(testgauge))
		...
		comp1 = as.vector(prin$loadings[,1] %*% t(testgauge2))
		comp2 = as.vector(prin$loadings[,2] %*% t(testgauge2))
		...
		for (k in 1:dim(trainout)[2]) {
			for (l in 0:39) {
				pred[l+1, k] = median(trainout[clust == l, k], na.rm = TRUE) 
			}
		}
